...
def chat(agent:Agent):
    ...
    recorder_config = {
        ...
        # 'continuous': True,  # Remove or comment out this line
    }
    ...
    while True:
        ...
        if user_input.lower() == 'e':
            ...
            clear_console()
            print("Say something... (Press Enter to send)")
            
            final_transcription = [""]

            def process_text(text):
                final_transcription[0] = text
                text_detected(text)

            recording_thread = threading.Thread(target=recorder.text, args=(process_text,))
            ...
            user_input = final_transcription[0]
            if user_input:
